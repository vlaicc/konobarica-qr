...
async function generateQRWithIcon(filename, iconSVG) {
    ...
    // Add white padding around QR code (~7.5mm = ~60px at print resolution)
    const padding = 60;
    ...
    if (iconSVG === 'wifi') {
        ...
        ctx.beginPath();
        ctx.arc(centerX, centerY + 30, 26, Math.PI * 1.25, Math.PI * 1.75, false);
        ctx.stroke();

        // Second arc (middle)
        ctx.beginPath();
        ctx.arc(centerX, centerY + 30, 46, Math.PI * 1.25, Math.PI * 1.75, false);
        ctx.stroke();

        // Third arc (largest) - outer
        ctx.beginPath();
        ctx.arc(centerX, centerY + 30, 66, Math.PI * 1.25, Math.PI * 1.75, false);
        ctx.stroke();
